...
function App() {
  ...
  const handleToggleSession = async (hour: number, isAdding: boolean) => {
    ...
    if (isAdding) {
      ...
      const originalSessions = [...sessions]; // Keep a copy for potential rollback
      ...
      try {
        ...
        // 3. If it fails, revert the local state change by restoring the original array
